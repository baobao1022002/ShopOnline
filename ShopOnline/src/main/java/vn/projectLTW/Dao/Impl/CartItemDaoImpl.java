...
public class CartItemDaoImpl implements ICartItemDao{
	...
	public CartItem findOne(String id) {
		String sql="SELECT CartItem.cartItemId,CartItem.quantity,CartItem.unitPrice,cart.userId,cart.buyDate,cart.status,cart.cartId,product.productName,product.price\r\n"
				+ "				 FROM CartItem INNER JOIN Cart ON Cart.cartId=CartItem.cartId INNER JOIN Product ON CartItem.productId=Product.productId\r\n"
	...
	public List<CartItem> findAllByUser(int id) {
		...
		String sql="SELECT CartItem.cartItemId,CartItem.quantity,CartItem.unitPrice,CartItem.cartId,CartItem.productId,Cart.userId,Cart.buyDate,Cart.status,Cart.cartId AS cart_id,Product.productName,Product.price,Product.images,Seller.sellerName,Seller.sellerId AS s_id,Seller.images AS avatar\r\n"
				+ "				 FROM CartItem INNER JOIN Cart ON Cart.cartId=CartItem.cartId\r\n"
	...
	public int countByUser(int id) {
		String sql="SELECT count(*) FROM CartItem INNER JOIN Cart ON CartItem.cartId=Cart.cartId where Cart.userId=?";
		try {
			conn=new DBConnection().getConnection();
			ps=conn.prepareStatement(sql);
			ps.setInt(1, id);
